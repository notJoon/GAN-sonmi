...
############# DISCRIMINATOR #############
    ...
    def build_discriminator(self):
        ...
        for i in range(self.n_layers_discriminator):
            ...
            if self.discriminator_batch_norm_momentum and i > 0:
                model = BatchNormalization(
                    momentum=self.discriminator_batch_norm_momentum)(model)
        ...
        discriminator_output = Dense(
            1, activation='sigmoid', kernel_initializer=self.weight_init)(model)
...
############# GENERATOR #############
    ...
    def _build_generator(self):
        ...
        model = Dense(np.prod(self.generator_init_dense_layer_size),
                      kernel_initializer=self.weight_init)(model)

        if self.generator_batch_norm_momentum:
            model = BatchNormalization(
                momentum=self.generator_batch_norm_momentum)(model)
...
############# TRAIN METHODS #############
    ...
    def train_discriminator(self, x_train, batch_size, using_generator):
        ...
        disc_loss_real, disc_acc_real = self.discriminator.train_on_batch(
            true_imgs, valid)
        disc_loss_fake, disc_acc_fake = self.discriminator.train_on_batch(
            generate_imgs, fake)
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            if epoch % print_every_nth_batches == 0:
                ...
                self.model.save_weights(os.path.join(
                    run_folder, f'weights/weights-{epoch}.h5'))
                self.model.save_weights(os.path.join(
    ...
    def sample_images(self, run_folder):
        ...
        for i in range(rows):
            for j in range(cols):
                axis[i, j].imshow(np.squeeze(
                    gen_imgs[counter, :, :]), cmap='gray')
                axis[i, j].axis('off')
                counter += 1
